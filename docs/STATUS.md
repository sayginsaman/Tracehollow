...
| Live source verification | **three connectors added** | Telegram's public web preview live-verified 2026-09-19 against Telegram's own announcements channel (`partial` at the one-page limit, 20 posts each with a datetime). YouTube's channel uploads and video comments live-verified the same day against YouTube's own channel and the first video published on the platform, with a key restricted to the Data API in a project created for the purpose; the first attempt failed on a one-page bound and is recorded as a failed check. Public web page, RSS/Atom and GitHub were already live-verified on 2026-09-15. Six of nine connectors now have a live check; Instagram needs Meta App Review and Subfinder's second provider still refuses |
